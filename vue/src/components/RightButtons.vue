<template>
  ...
  <v-dialog v-model="showPopup" max-width="500">
    ...
    <v-card style="height: 90vh">
      <v-card-title> What would you like to track? </v-card-title>
      <v-card-text>
        <v-row>
      ...
      <v-dialog v-model="showWorkout" max-width="500">
        <v-card>
          <v-card-title> Workout </v-card-title>
          <v-card-text>
            <v-form>
              <v-select
            ...
            <div v-if="selectedWorkout === 'Bench'">
              <v-form>
                <v-text-field
                  ...
                  type="number"
                />
                <v-text-field label="Reps" v-model="benchReps" type="number" />
              </v-form>
            </div>
      ...
      <v-dialog v-model="showTask" max-width="450">
        <v-card>
          ...
          <v-card-text>
            ...
            <v-text-field label="Reward" v-model="bounty"></v-text-field>
            <v-row class="mx-2 d-flex justify-content-center">
              <v-btn color="primary" @click="setTaskType('today')">Today</v-btn>
              <v-btn color="secondary" @click="setTaskType('daily')"
                >Daily</v-btn
              >
            </v-row>
            <v-row class="mx-2 d-flex justify-content-center">
              <v-btn color="blue" @click="setTaskType('Routine')"
                >Routine</v-btn
              >
            </v-row>
          </v-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      selectedCardioType: "",
      duration: "",
      cardioTypes: ["Running", "Walking", "VR"],
    };
  },
  methods: {
      ...
      for (let i = 0; i < this.items.length; i++) {
        if (this.counts[i] !== 0) {
          ...
          if (
            this.items[i] === "Water" ||
            this.items[i] === "Sex" ||
            this.items[i] === "Night Vit" ||
            this.items[i] === "Morning Vit"
          ) {
            log = {
              userid: this.userId,
              date: new Date(),
